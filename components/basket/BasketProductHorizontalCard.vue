<template>
	<div class="flex w-full p-2 rounded-2xl">
		<img :src="$apiUrl.BaseUrl() + $apiUrl.GetMediaUrl(product.main_image)" alt="" class="w-16 h-16 rounded-2xl"  />
		<div class="flex flex-col ms-2 flex-grow me-3 text-natural-dark">
			<div class="flex flex-row items-center">
				<span class="text-sm font-black me-4">
					{{ product.title || $strings.without_name() }}
				</span>
				<span class="flex-grow" />
				<span class="text-xs font-medium me-1"> {{ product.quantity || 1 }} </span>
				<span class="text-xs font-medium me-1"> * </span>
				<span class="text-2xl font-black me-1">
					{{ formattedPrice }}
				</span>
				<span class="text-xs self-start font-medium">
					{{ $strings.toman() }}
				</span>
			</div>
			<div class="flex flex-row">
				<span class="flex items-center py-1 px-4 rounded bg-white ms-2">
...
export default class BasketProductHorizontalCard extends Vue {
	...
	get category(): ShopCategory | null {
		return this.categories.find((v) => v.id == this.product.category) || null
	}
