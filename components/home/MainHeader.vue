<template>
	<div class="flex flex-col bg-white items-center px-6">
		<div class="flex items-center max-w-screen-xl w-full py-6">
			...
			<template v-else-if="type === 'shop'">
				...
				<nuxt-link
					...
					class="w-10 h-10 bg-primary bg-opacity-10 rounded-full flex items-center justify-center cursor-pointer relative hidden lg:flex"
					:class="{
		...
		<TransitionFade>
			<div
				v-if="isMenuVisible"
				class="fixed left-0 right-0 top-0 h-screen z-20 bg-black bg-opacity-50 flex"
			...
			>
				<div
					@click="(e) => e.stopPropagation()"
					class="flex flex-col max-w-sm bg-white flex-grow shadow-xl overflow-y-auto"
				>
					...
					>
						...
						<template v-else-if="type === 'shop'">
							...
							<nuxt-link
								:to="$routeUrl.ShopBasketUrl()"
								class="flex w-10 h-10 bg-primary bg-opacity-10 rounded-full flex items-center justify-center cursor-pointer relative"
								:class="{
									'text-primary-dark border-2 border-primary':
										basket.length,
									'text-natural-dark': !basket.length,
						...
						<span
							v-if="!user"
							class="flex w-px h-8 bg-gray-200 mx-4"
						/>
						<nuxt-link
							v-if="!user"
							:to="$routeUrl.LoginUrl()"
					...
					<div
						v-if="user"
						class="h-px bg-gray-200 mx-4 flex-shrink-0"
					/>
					<UserMenu
						v-if="user"
