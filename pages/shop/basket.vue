<template>
	<BasketContainer :step="step" @back="back" @submit="submit" :loading="loading">
		<div class="flex flex-col px-6 items-center flex-grow">
			<div class="w-full max-w-screen-xl flex flex-col mt-14 mb-16 flex-grow">
				<BasketBuySection
					v-if="step === 1"
					:products="basket"
...
export default class BasketPage extends Vue {
	...
	async submit() {
		if (this.step === 1) {
			...
			if (!this.user || !this.user?.first_name || !this.user?.last_name || !this.user.phone) {
				this.$toast.error(
